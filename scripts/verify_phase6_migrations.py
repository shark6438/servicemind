...
def main() -> None:
    ...
    database_name = f"servicemind_phase6_{uuid4().hex[:12]}"
    ...
    try:
        ...
        with psycopg.connect(runtime_url) as connection:
            ...
            table_count = connection.execute(
                ...
                "'tool_policy_decisions','governed_tool_invocations','tool_outbox','mcp_tasks')"
            ).fetchone()
        assert revision is not None and revision[0] == "0013_phase6_hash_guards"
        assert table_count is not None and table_count[0] == 8
        print(
            "PASS phase6 fresh migration: upgrade -> full downgrade -> upgrade, head=0013, tables=8"
        )
    finally:
        with psycopg.connect(admin_url, autocommit=True) as connection:
            connection.execute(
