...
3. TechQA 400 条公开代理评测已经执行。**首次运行快照（已过时）**：Recall@5=0.6536、Recall@10=0.7000、MRR@10=0.5541、NDCG@10=0.5896；四项点估计均低于预设目标阈值（代理集上不作正式 PASS/FAIL）。
   **当前值**（`evaluation/reports/phase4_proxy_release_latest.json`，schema `phase4-proxy-release-v1.4`）：生产形态臂（top-100 候选池 + `0.85×重排分 + 0.15×minmax(检索分)` 融合）Recall@5=0.6857、Recall@10=0.7643、Recall@20=0.8071、MRR@10=0.5848、NDCG@10=0.6279；已发布协议臂（top-30 池 + 纯重排，保留以便历史可比）Recall@5=0.6786、Recall@10=0.7321、Recall@20=0.7857、MRR@10=0.5678、NDCG@10=0.6077。两臂均在**生产重排窗口**（pinned checkpoint 自身的 8192 token）下测得；门禁项在本代理集上不适用（见 `docs/PHASE4_RAG_QUALITY_ROOT_CAUSE_2026-09-15.md` §2.3）。
   报告同时给出两个口径（生产形态臂与已发布协议臂），两者差额即口径差额：Recall@10 **+0.0321**、NDCG@10 **+0.0206**（配对自助 95% CI 分别为 `[+0.0071, +0.0571]` 与 `[+0.0006, +0.0414]`，在两个闭区间上端点均大于 0；按同一 400 条 query 在离线 TEI 路径上计算，§2.7 已证明该路径与框架生产臂逐位等价），即生产口径确实优于旧发布口径，而不是噪声。它被保留为跨域鲁棒性基线，不冒充租户人工金标；上述生产口径数字与独立的离线 TEI 重排路径**十个数字逐位相同**（同一文档 §2.7）。
   上一条与上上条中出现的旧数字（Recall@5=0.6536/0.6821 等）都是**重排序列窗口被写死为 512** 的旧协议产物——生产继承 checkpoint 的 8192 窗口，而评测只喂了 512。实测子块 p50 为 **1313 字符 / 417 token**，单独子块均未超过 512；窗口影响来自 query+passage 拼接对，抽样 **27.5%** 超过 512 token。该测量层缺陷已修复，并在生产窗口下全量重跑刷新（同一文档 §2.6、§3.6）；§4.1 各门禁在新协议下**仍只是点估计且低于目标阈值**（代理集上门禁不适用，
只有租户专家 qrels 才能形成正式 PASS/FAIL），结论方向不变。
...
每项都报告总体值、各类别值和 95% CI。

**关闭门只在租户域 release set 上形成正式 PASS/FAIL。** 上表阈值保持不变，但它们在外部
silver 代理集（TechQA）上**不构成裁定**：报告里这些字段为 `applicable: false / passed: null`，
只给出点估计。代理集上的点估计低于阈值**不等于**「门禁未通过」，反之亦然。

#### 4.1.1 新增检索臂的采纳规则（多目标，2026-09-15 修订）

任何新增检索臂（额外稠密/稀疏召回分支、新重排器、新融合口径）必须**同时**通过下列四组
门禁才可进入生产；任一组不过即维持现役配置。本规则取代原「困难子集 NDCG@10 绝对提升
≥ 2 个百分点且 CI 下界 > 0」的单指标判据（修订理由见
`docs/PHASE4_RAG_QUALITY_ROOT_CAUSE_2026-09-15.md` §3.3 与 §4 末）。

**一、收益门禁** —— 下列三项须全部满足：

1. 全体 Recall@5 提升，且 paired-bootstrap 95% CI 下界 > 0；
2. 预先冻结的困难集 Recall@5 提升，且 CI 下界 > 0；
3. 困难集定义必须**在看到候选臂结果之前**冻结，且**在未参与选型的数据上**计算。
   由现役系统失败样本事后划定的困难集，只能作**目标子集**指标，不得充当独立证据。

**二、排序非劣门禁（双档）** —— 判据如下：

Recall@10、MRR@10、NDCG@10 须通过单侧 95% 非劣检验（配对自助），即 CI 下界 > −δ：

| 档位 | 绝对非劣界 δ | 依据 | 状态 |
| --- | ---: | --- | --- |
| 当轮（可用标注数据） | **0.020** | 检验力论证：n=330 可答 query 时配对差 SE≈0.010，δ=0.020 是「真效应为 0 时仍可判定」的最小整数界 | **生效** |
| Phase 7 | 0.005 | 需租户专家 qrels 或等规模新标注集；现有 610 条可答 query 的 CI 半宽 ≥0.0145，该界**在现有数据上不可分辨** | 未生效 |

当轮档通过只等于「在可用样本量能听见的分辨率下未见退化」，**不得表述为更强的结论**。
δ 的选定依据必须是**检验力**，不是观测到的效应量——见到候选臂结果之后再上调 δ 属于
事后选界，等同于追认。

**三、运行成本门禁** —— 三项都必须在预算内：

- p95 检索延迟增幅 ≤ 20%；
- GPU 显存、索引容量与重建时间在预算内；
- 跨 tenant / ACL 泄漏、失效版本召回继续为 **0**。

**四、生成与弃答单独验收** —— 取证要求：

弃答能力**不再**用「金标是否在候选池内」代替：必须用**真实 Reviewer 输出**测
answerable success、错误作答率、过度拒答率与 citation 正确性。候选池覆盖率只说明
理论空间（且池覆盖率是 oracle 量），不构成弃答率证据。

**五、禁止追认** —— 下列顺序不可颠倒：

1. 先冻结本规则与 δ；
2. 再用**未参与本轮分析的 holdout** 或 shadow 取证验证；
3. 不得在修改规则之后，用已经反复分析过的同一批数据直接追认候选臂。

#### 4.1.2 两类证据来源不可互相替代

| 门禁 | 取证来源 | 原因 |
| --- | --- | --- |
| 一（收益）、二（非劣） | **有标签的新留出集** | shadow 流量没有 qrels，算不出 Recall/NDCG |
| 三（成本/SLO）、四（生成与弃答） | shadow / canary 流量 | 不需标签，但需真实负载与真实 Reviewer |

TechQA-RAG-Eval 尚有 **330 条未参与本轮分析的可答 query**（910 行 − 已用 280，
`_select()` 为固定 seed 的确定性抽样）可作**一次性**留出集；用尽后该数据集再无第三个
判决集，必须取得租户 qrels。

#### 4.1.3 shadow 的工程约束

- **只读影子**：候选臂排名只记录、不服务、不改写现役 alias；
- **独立索引代**：第三稠密臂须建独立 `...-children-{gen}` 并只在影子侧挂 alias，
  现役 alias 与生产检索路径不受影响；
- shadow 期间 §6.4 代理回归门禁继续对**现役**配置执行，不得因影子实验放宽。

> 第三检索臂（Qwen3-Embedding-0.6B）已按本规则于 2026-09-15 施加预注册门禁，并在
> **一次性留出集**上判定为 `FAIL`。**项目方 2026-09-16 裁定：本候选的确认性验证正式关闭**，
> 最终状态 `CONFIRMATORY_FAIL / PRODUCTION_NOT_APPROVED / SHADOW_NOT_AUTHORIZED /
> RESEARCH_ARCHIVED`，**不进入 shadow**，生产保持现役两路配置。
> 判读、勘误与重启条件见 `docs/PHASE4_RAG_QUALITY_ROOT_CAUSE_2026-09-15.md` §3.3；
> 归档见 `evaluation/experiments/phase4_third_arm_2026-09-15/` 与
